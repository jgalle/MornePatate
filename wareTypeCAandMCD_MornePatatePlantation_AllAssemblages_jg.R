...
# wareTypeCAandMCD_MornePatatePlantation_Phase.R
...
# Edited by:   JEG 12.28.2018 Changed File Name to better reflect what the code does
...
# Replace Unid. Carib CEW ware type with 'Caribbean CEW Unid.'
wareTypeData <- mutate(wareTypeData, Ware=ifelse(Ware == 'Morne Patate Type 1b', 
                                                 'Morne Patate Type 1 Combined', Ware))
# Replace Unid. Carib CEW ware type with 'Caribbean CEW Unid.'
wareTypeData <- mutate(wareTypeData, Ware=ifelse(Ware == 'Morne Patate Type 1a', 
                                                 'Morne Patate Type 1 Combined', Ware))
# Replace Unid. Carib CEW ware type with 'Caribbean CEW Unid.'
...
p <- ggplot(data=inertia , aes(x= 1:length(Inertia), y=Inertia)) +
  ...
  labs( title="Morne Patate Plantation", x="Dimension", y='Porportion of Inertia' ) +
  geom_line(aes(y = bs[,2], x= bs[,1]), color = "black", linetype = "dashed", 
            size=1)
p

# get the projectID into the rowScores so we can color code the scatter plots by Project 
projectKey <- wareTypeData_Unit %>% distinct(ProjectID,ProjectName, unit) %>%
  mutate(Project = case_when(
        ProjectID == '1243' ~ 'Village',
        ProjectID == '1250' ~ 'Estate',
        ProjectID == '1251' ~ 'Provision Grounds' ))
rowScores <- inner_join(projectKey, rowScores, by='unit') 


# ggplot version of row scores dim 1 and dim 2
library(ggrepel)
set.seed(42)
p1 <- ggplot(rowScores, aes(x=Dim1,y=Dim2, fill= Project))+
...
p5a <- p5 + geom_vline(xintercept=c(.8), colour = "gray", linetype = "dashed",
            size=1)
p5a
...
rownames(matX) <- wareByUnitT_forCA_Late1$unit
...
projectKey <- wareTypeData_Unit %>% distinct(ProjectID,ProjectName, unit) %>%
  mutate(Project = case_when(
    ...
    ProjectID == '1250' ~ 'Estate',
    ProjectID == '1251' ~ 'Provision Grounds' ))
rowScores1 <- inner_join(projectKey, rowScores, by='unit') 


# ggplot version of row scores dim 1 and dim 2
library(ggrepel)
set.seed(42)
p1 <- ggplot(rowScores1, aes(x=Dim1,y=Dim2, fill= Project))+
  geom_point(shape=21, size=5, colour="black", alpha=.5) +
  scale_fill_viridis(discrete =T) +
  # geom_text(aes(label= unit,vjust=-.6, cex=5) +
  # geom_text_repel(aes(label= unit), cex = 4) +
...
################################# Fraser's changes end here #########################


#### 14. Compare MCD and CA dim scores
# create a data frame of units, counts, and mcds
CA_MCD1 <- inner_join(MCDByUnit$MCDs, rowScores1, by='unit' )

# Plot CA Dim 1 vs. MCDs
#ggplot version of CA Dim 1 vs. MCDs
p3 <- ggplot(CA_MCD1, aes(x=Dim1,y=blueMCD))+
  geom_point(shape=21, size=5, colour="black", fill="cornflower blue")+
  #geom_text(aes(label=unit),vjust=-.6, cex=5)+
  #geom_text_repel(aes(label=unit), cex=6) +
  labs(title="Morne Patate Plantation", 
...
p4 <- ggplot(CA_MCD1, aes(x = Dim2,y = blueMCD))+
  geom_point(shape=21, size=5, colour="black", fill="cornflower blue")+
  #geom_text(aes(label=unit),vjust=-.6, cex=5)+
  #geom_text_repel(aes(label=unit), cex=6) +
  labs(title="Morne Patate Plantation", 
...
#### 15. Histogram of Dim 1 scores for Phasing
# Dim 1 Scores Weighted Histogram, you may need to change scale
dim1ForHist<- data.frame(dim1 = rep(CA_MCD1$Dim1, CA_MCD1$Count))
p5 <- ggplot(dim1ForHist, aes(x = dim1)) +
  geom_histogram(aes(y=..density..), colour="black", fill="tan", binwidth=0.2, 
                 boundary= .1) +
  labs(title="Morne Patate Plantation", x="Dimension 1", y="Density") +
...
# Add lines for phase breaks
p5a <- p5 + geom_vline(xintercept=c(-.9,0,1), colour = "gray", linetype = "dashed",
                       size=1)
p5a
...
# ggsave("MPPlantation_Dim1MCDcolor_2018.png", p6, width=10, height=7.5, dpi=300)
...
### Everything below here will not work until I put phases into DAACS
...
write.csv(phaseAssignments, file='ContextPhases_MPPlantation.csv')
