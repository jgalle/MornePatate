...
# wareTypeCAandMCD_MornePatate_BySite_Phase.R
...
p <- ggplot(data=inertia , aes(x= 1:length(Inertia), y=Inertia)) +
  ...
  labs( title="Morne Patate Estate", x="Dimension", y='Porportion of Inertia' ) +
...
p1 <- ggplot(rowScores, aes(x=Dim1,y=Dim2))+
  ...
  labs(title="Morne Patate Estate", 
...
p5 <- ggplot(dim1ForHist, aes(x = dim1)) +
     ...
     # scale_x_continuous(breaks=seq(- 8 , 4, 2))+
      ...
      labs(title="Morne Patate Estate", x="Dimension 1", y="Density") +
      geom_density(fill=NA)
p5


# Add lines for phase breaks
p5a <- p5 + geom_vline(xintercept=c(-.8, .6 ), colour = "gray", linetype = "dashed",
            size=1)
p5a
...
CA_MCD_Phase <- CA_MCD %>% mutate( Phase = case_when (Dim1 <= -.8 ~ 'P03',
                                      (Dim1 > -.8) & 
                                      (Dim1 <= .6) ~ 'P02',
                                       Dim1 > .6 ~ 'P01'
                                      ))
...
# ggsave("MPEstate_Dim1MCDcolor_2018.png", p6, width=10, height=7.5, dpi=300)
...
write.csv(phaseAssignments, file='ContextPhases_MPEstate.csv')
